...
Version one is manual entry plus that blind colour-comparison test. No photo measurement, no wardrobe cataloguing, no Korean tone lens.
Face shape guidance exists, off by default, labelled as a rule of thumb rather than a finding.
See `DECISIONS.md` for the full scope record and reasoning.

This suggests, it does not determine.
Every result is probabilistic and confidence-banded, and the comparison test exists precisely so your own eyes settle what a season label can't.
...
It deploys to GitHub Pages as a project page, from this public repository.
