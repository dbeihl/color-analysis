...
The set's own README names the three source documents by filenames that do not match the actual files, which breaks the one workflow it is written for. Anyone trying to find the source documents from that README first should know this before they go looking.

## Corrections

Each correction is marked with whether it applies to version one now, or only once the deferred photo path is built.

### 1. Palette source — applies now
...
### 2. Calibration reference — deferred, photo path only
...
Part of the evidence for this method did not survive verification; see "What was checked in the research, and what did not survive" below before implementing it.
...
### 4. Accuracy ceiling and confidence — deferred, photo path only
...
Correction, sharper version: the best documented automated accuracy on the four main colour seasons, from the Deep Armocromia work at the ECCV 2024 workshops, is 0.554. This tool has twelve seasons, so twelve-class accuracy is necessarily worse.
