...
- `src/knowledge/seasons.json` is generated by `scripts/derive-palettes.py` and is never hand-edited; `npm run derive:check` fails on any drift. `docs/palette-derivation.md` owns the derivation, its regions and its open limitations; `docs/resolver.md` owns the classifier's chosen thresholds and its uncertainty contract.
